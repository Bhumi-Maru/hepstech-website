...
export default function Home_Page_Section_6() {
  ...
  const sectionBanners = homePage.filter((banner) => {
    return banner.home_page_layout_number?.layoutNumber === 6;
  });
  ...
  const handleEdit = (banner) => {
    ...
    setFormData({
      layoutNumber: banner.home_page_layout_number?.layoutNumber || 6,
  ...
  const handleSave = async () => {
    try {
      ...
      await saveSectionTitle(formData.sectionTitle, 6);
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex-shrink-0 ml-4 toggle-switch">
            <input
              ...
              role="checkbox"
              tabindex="0"
              checked={toggleStates.category_Slider_02}
              onChange={() => handleToggle("category_Slider_02")}
            />
            <label for="categorySection02Status"></label>
          </div>
        </div>

        {toggleStates.category_Slider_02 && (
          <>
            <div
              className="mt-4"
              id="categorySection02Content"
              style={{ display: "block" }}
            >
