...
export default function Footer() {
  ...
  //         // Sort by createdAt in descending order to get the latest first
  //         const sortedData = [...response.data.data].sort(
  //           (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
  //         );
  //         console.log("sorted data", sortedData);
  //         // Get the most recent footer data
  //         setFormData(sortedData[0]);
  //       }
  //     } catch (error) {
        ...
        <div className="container">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-12 gap-y-10 gap-x-8">
            <div className="col-span-2 md:col-span-4 lg:col-span-3">
              ...
              {formData?.contactDetails?.address && (
                <>
                  <p className="mt-4">{formData?.contactDetails?.address}</p>
                </>
              )}
              ...
              <ul className="flex items-center mt-6 space-x-3">
                {formData?.socialMediaLinks?.Facebook && (
                  <>
                    <li>
                      <a
                        href={formData.socialMediaLinks.Facebook}
                        title="Facebook"
                ...
                {formData?.socialMediaLinks?.Twitter && (
                  <>
                    <li>
                      <a
                        href={formData.socialMediaLinks.Twitter}
                        title="Twitter"
                ...
                {formData?.socialMediaLinks?.Instagram && (
                  <>
                    <li>
                      <a
                        href={formData.socialMediaLinks.Instagram}
                        title="Instagram"
                ...
                {formData?.socialMediaLinks?.Pinterest && (
                  <>
                    <li>
                      <a
                        href={formData.socialMediaLinks.Pinterest}
                        title="Pinterest"
                ...
                {formData?.socialMediaLinks?.LinkedIn && (
                  <>
                    <li>
                      <a
                        href={formData.socialMediaLinks.LinkedIn}
                        title="LinkedIn"
            ...
            <div className="lg:col-span-3">
              ...
              <ul className="mt-6 space-y-4">
                ...
                {/* timings */}
                {formData?.contactDetails?.timings && (
                  <>
                    <li className="flex items-start transition-all duration-200 transform hover:text-opacity-80 hover:translate-x-1">
                      <svg
                        className="flex-none w-5 h-5 mr-3"
                ...
                {formData?.contactDetails?.Email_Address && (
                  <>
                    <li className="flex items-start transition-all duration-200 transform hover:text-opacity-80 hover:translate-x-1">
                      <svg
                        className="flex-none w-5 h-5 mr-3"
            ...
            {/* COLUMN 1 */}
            {/* Dynamic Columns */}
            {formData?.columnsData?.map((column, columnIndex) => (
                ...
                <ul className="mt-6 space-y-3">
                  {column.links.map((link, linkIndex) => (
                    <li key={linkIndex}>
                      <a
                        href={
                          link.type === "page" ? `/page/${link.url}` : link.url
                        }
                        title={link.title}
          ...
          <div className="text-center md:text-left md:flex md:items-center md:justify-between">
            ...
            <div className="md:order-2">
              {formData?.PaymentOptionsPhoto?.length > 0 ? (
                <div className="flex flex-wrap justify-center gap-4">
                  {formData.PaymentOptionsPhoto.map((payment, index) => (
                    <img
                      key={index}
                      className="w-auto h-5"
                      src={`http://localhost:7000${payment.filePath}`}
