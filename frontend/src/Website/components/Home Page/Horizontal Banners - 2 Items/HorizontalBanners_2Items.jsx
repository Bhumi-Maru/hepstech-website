...
export default function HorizontalBanners_2Items() {
  ...
  // Filter all banners with layoutNumber === 10
  const banners = homePage.filter(
    (item) =>
      item.home_page_layout_number.layoutNumber === 10 &&
          ...
          <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            {[0, 1].map((index) => {
              ...
              return (
                <div
                  key={index}
                  class="overflow-hidden bg-gray-200 border border-gray-200 rounded-lg aspect-w-16 aspect-h-5"
                >
                  {banner ? (
